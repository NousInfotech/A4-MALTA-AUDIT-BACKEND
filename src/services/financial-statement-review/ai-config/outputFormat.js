...
1. TEST ID MAPPING: Use the test list provided to get test_id and category. Format test_id as "T" + number (e.g., "T1", "T4") or use the test_id format from the test list (e.g., "AR01", "BS01", "PRES01"). Use test_name for area field. Category must be one of: AUDIT_REPORT, BALANCE_SHEET, INCOME_STATEMENT, GENERAL, NOTES_AND_POLICY, CROSS_STATEMENT, PRESENTATION.

2. SECTION A: title exactly "CONFIRMED CORRECT ITEMS". Include only tests that passed completely. Fields: test_id (format from test list), category (required - one of the valid categories: AUDIT_REPORT, BALANCE_SHEET, INCOME_STATEMENT, GENERAL, NOTES_AND_POLICY, CROSS_STATEMENT, PRESENTATION), area (human-readable name), details (brief verification text). CRITICAL: A test_id that appears in Section B or C MUST NOT appear in Section A.

3. SECTION B: title exactly "CRITICAL ERRORS". Fields: id (B1, B2, B3 sequential), test_id (format from test list), category (required - one of the valid categories: AUDIT_REPORT, BALANCE_SHEET, INCOME_STATEMENT, GENERAL, NOTES_AND_POLICY, CROSS_STATEMENT, PRESENTATION), type (arithmetical/logic/portal_crosscheck/registry_crosscheck/visual/structure/disclosure/classification/legal/grammar/presentation), severity always "critical", description, location (page number or null, section string or null, note string or null, line_hint string or null), reported_value (number/string/null), expected_value (number/string/null), difference (number or null), reason, financial_impact, suggested_fix. Include all arithmetical errors, logic errors, material misstatements. CRITICAL: A test_id that appears in Section B MUST NOT appear in Section A or Section C.

4. SECTION C: title exactly "DISCLOSURE & REGULATORY BREACHES". Fields: id (C1, C2, C3 sequential), test_id (format from test list), category (required - one of the valid categories: AUDIT_REPORT, BALANCE_SHEET, INCOME_STATEMENT, GENERAL, NOTES_AND_POLICY, CROSS_STATEMENT, PRESENTATION), type, severity (regulatory or presentation), description, location (same as B), impact (NOT financial_impact), suggested_fix. Include missing disclosures, regulatory non-compliance, presentation issues. CRITICAL: A test_id that appears in Section C MUST NOT appear in Section A or Section B.

CRITICAL RULE - TEST EXCLUSIVITY: Each test_id can ONLY appear in ONE section. If a test has ANY critical error (Section B), it cannot be in Section A or C. If a test has ANY regulatory breach (Section C) but no critical errors, it cannot be in Section A or B. Only include a test in Section A if it has passed completely with no errors or breaches found.

CRITICAL RULE - CATEGORY FIELD: Every test item in Sections A, B, and C MUST include a "category" field. The category must match the test category from the test list provided. Valid categories are: AUDIT_REPORT, BALANCE_SHEET, INCOME_STATEMENT, GENERAL, NOTES_AND_POLICY, CROSS_STATEMENT, PRESENTATION. The category field is REQUIRED for all test items. IMPORTANT: Tests with test_id starting with "PRES" (e.g., "PRES01") MUST have category "PRESENTATION", not "GENERAL" or any other category.
