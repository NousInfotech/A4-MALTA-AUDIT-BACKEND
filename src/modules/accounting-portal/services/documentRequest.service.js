...
async function getDocumentRequestsByEngagementId(engagementId) {
  try {
    const documentRequests = await DocumentRequest.find({ engagement: engagementId })
      .sort({ requestedAt: -1 });
 ...
 */
async function uploadDocumentRequestDocument(documentRequestId, files, body, user) {
  try {
    ...
    for (const file of files) {
      ...
      if (dr.category === 'kyc') {
        ...
        const documentIndex = body.documentIndex ? parseInt(body.documentIndex) : null;
    ...
    if (dr.category === 'kyc') {
      try {
        const kyc = await KYC.findOne({ 'documentRequests.documentRequest': dr._id });
        if (kyc) {
